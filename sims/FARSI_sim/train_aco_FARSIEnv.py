...
import os
import sys
#from Project_FARSI.design_utils.components.workload import Workload

os.sys.path.insert(0, os.path.abspath('../../'))
print(os.sys.path)
from aco.DeepSwarm.deepswarm.backends import Dataset, FARSIBackend
...
flags.DEFINE_integer('depth', 16, 'Depth of the network.')
...
def main(_):
    ...
    log_dir = os.path.join(FLAGS.summary_dir, FLAGS.aco_log_dir, reward_formulation, exp_name)

    print("Trajectory directory:", traject_dir)
    print("Log directory:", log_dir)
    print("Experiment name:", exp_name)
    
    backend = FARSIBackend(dataset, 
                            exp_name=exp_name,
                            log_dir=log_dir,
